...
#this is the class I am looking for
#need to find each characters probability
def translate_back(outputs,threshold=0.7):
    ...
    #outputs is the thing we need to find
    #print len(outputs[:,0])
    #print len(outputs[0])#156 character probability
    #print outputs[0,0]
    #print outputs[:,0]<threshold
    #print len(outputs[1])#how many is one character
...
#########################  
    ...
    print "#######!!"
    f = measurements.find_objects(labels)
    for j in range(0,len(trueanswer)):
      for i in range(17,27):
        print max(outputs[f[j]][:,i])
      print '\n'
    print "after"
##############
    for j in range(0,len(trueanswer)):
      for i in range(0,156):
        if trueanswer[j]==' ':
	  break
        elif i==int(trueanswer[j])+17:
          outputs[f[j]][:,i] = outputs[f[j]][:,i]*0.6
        elif i in range(17,27):
          outputs[f[j]][:,i] = outputs[f[j]][:,i]*(0.4/9)
        else:
          outputs[f[j]][:,i] = -1
      print '\n'
###########################    
   ...
   # print "measurements.maximum(outputs,mask,arange(1,amax(mask)+1))"
  #  print measurements.maximum(outputs,mask,arange(1,amax(mask)+1))
   # print "measurements.maximum_position(outputs,mask,arange(1,amax(mask)+1))"
   # print measurements.maximum_position(outputs,mask,arange(1,amax(mask)+1))
...
class SeqRecognizer:
	...
	out = translate_back(self.outputs)
        return out
    ...
    def predictString(self,xs):
        ...
        cs = self.predictSequence(xs)#I have got result here so must before here.So must be in predictSequence
        return self.l2s(cs)
